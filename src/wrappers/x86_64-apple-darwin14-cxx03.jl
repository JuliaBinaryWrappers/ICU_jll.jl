# Autogenerated wrapper script for ICU_jll for x86_64-apple-darwin14-cxx03
...
const libicudata_splitpath = ["lib", "libicudata.67.1.dylib"]
...
const libicudata = "@rpath/libicudata.67.dylib"


# Relative path to `libicui18n`
const libicui18n_splitpath = ["lib", "libicui18n.67.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libicui18n = "@rpath/libicui18n.67.dylib"


# Relative path to `libicuio`
const libicuio_splitpath = ["lib", "libicuio.67.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libicuio = "@rpath/libicuio.67.dylib"


# Relative path to `libicutest`
const libicutest_splitpath = ["lib", "libicutest.67.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libicutest = "@rpath/libicutest.67.dylib"


# Relative path to `libicutu`
const libicutu_splitpath = ["lib", "libicutu.67.1.dylib"]
...
const libicuuc_splitpath = ["lib", "libicuuc.67.1.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libicuuc = "@rpath/libicuuc.67.dylib"
