...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import ICU_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "ICU_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"ICU", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "ICU_jll", "src", "ICU_jll.jl"))
        @info("ICU_ll dev'ed out to /depot/dev/ICU_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a51ab1cf-af8e-5615-a023-bc2c838bba6b"))
platforms = [Pkg.Artifacts.unpack_platform(e, "ICU", artifacts_toml) for e in artifacts["ICU"]]
...
if best_platform === nothing
    @debug("Unable to load ICU; unsupported platform $(triplet(platform_key_abi()))")
else
